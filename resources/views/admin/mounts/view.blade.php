...
@section('content-header')
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{ route('admin.mounts') }}">挂载点</a></li>
                ...
                <form action="{{ route('admin.mounts.view', $mount->id) }}" method="POST">
                    <div class="box-body">
                        <div class="form-group">
                            <label for="PUniqueID" class="form-label">唯一ID</label>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('button[data-action="detach-egg"]').click(function (event) {
                ...
                $.ajax({
                    method: 'DELETE',
                    url: '/admin/mounts/' + mountId + '/eggs/' + eggId,
                    headers: { 'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content') },
                }).done(function () {
                    element.parent().parent().addClass('warning').delay(100).fadeOut();
                    swal({ type: 'success', title: '预设已分离。' });
                }).fail(function (jqXHR) {
                    console.error(jqXHR);
                    swal({
                        title: '哎呀！',
            ...
            $('button[data-action="detach-node"]').click(function (event) {
                ...
                $.ajax({
                    method: 'DELETE',
                    url: '/admin/mounts/' + mountId + '/nodes/' + nodeId,
                    headers: { 'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content') },
                }).done(function () {
                    element.parent().parent().addClass('warning').delay(100).fadeOut();
                    swal({ type: 'success', title: '节点已分离。' });
                }).fail(function (jqXHR) {
                    console.error(jqXHR);
                    swal({
                        title: '哎呀！',
