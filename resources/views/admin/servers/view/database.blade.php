...
<div class="row">
    <div class="col-sm-7">
        <div class="alert alert-info">
            数据库密码可以在前端<a href="/server/{{ $server->uuidShort }}/databases">访问此服务器</a>时查看。
        </div>
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">活动数据库</h3>
            </div>
            ...
            <form action="{{ route('admin.servers.view.database', $server->id) }}" method="POST">
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <p class="text-muted small">选择应在此上创建此数据库的主机数据库服务器。</p>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="pRemote" class="control-label">连接</label>
                        <input id="pRemote" type="text" name="remote" class="form-control" value="%" />
                        <p class="text-muted small">这应该反映允许连接的IP地址。使用标准MySQL表示法。如果不确定，请保留为<code>%</code>。</p>
                    </div>
                    <div class="form-group">
                        <label for="pmax_connections" class="control-label">并发连接</label>
                        <input id="pmax_connections" type="text" name="max_connections" class="form-control"/>
                        <p class="text-muted small">这应该反映此用户到数据库的最大并发连接数。留空表示无限制。</p>
                    </div>
                </div>
                <div class="box-footer">
                    {!! csrf_field() !!}
                    <p class="text-muted small no-margin">提交表单后，将为此数据库随机生成用户名和密码。</p>
