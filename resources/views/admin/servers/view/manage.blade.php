...
@section('content-header')
    <h1>{{ $server->name }}<small>控制此服务器的附加操作。</small></h1>
    <ol class="breadcrumb">
    ...
    <div class="row">
        <div class="col-sm-4">
            <div class="box box-danger">
                ...
                <div class="box-body">
                    <p>这将使用分配的服务脚本重新安装服务器。<strong>危险！</strong>这可能会覆盖服务器数据。</p>
                </div>
                <div class="box-footer">
                    ...
                    @else
                        <button class="btn btn-danger disabled">服务器必须正确安装才能重新安装</button>
                    @endif
                </div>
            </div>
        </div>
        <div class="col-sm-4">
            <div class="box box-primary">
                ...
                <div class="box-body">
                    <p>如果您需要将安装状态从未安装更改为已安装，反之亦然，可以使用下面的按钮进行操作。</p>
                </div>
                <div class="box-footer">
        ...
        @if(! $server->isSuspended())
            <div class="col-sm-4">
                <div class="box box-warning">
                    <div class="box-header with-border">
                        <h3 class="box-title">暂停服务器</h3>
                    </div>
                    <div class="box-body">
                        <p>这将暂停服务器，停止任何正在运行的进程，并立即阻止用户通过面板或API访问其文件或以其他方式管理服务器。</p>
                    </div>
                    <div class="box-footer">
                        <form action="{{ route('admin.servers.view.manage.suspension', $server->id) }}" method="POST">
                            {!! csrf_field() !!}
                            <input type="hidden" name="action" value="suspend" />
                            <button type="submit" class="btn btn-warning @if(! is_null($server->transfer)) disabled @endif">暂停服务器</button>
                        </form>
                    </div>
                </div>
            </div>
        @else
            <div class="col-sm-4">
                <div class="box box-success">
                    <div class="box-header with-border">
                        <h3 class="box-title">取消暂停服务器</h3>
                    </div>
                    <div class="box-body">
                        <p>这将取消暂停服务器并恢复正常的用户访问权限。</p>
                    </div>
                    <div class="box-footer">
                        <form action="{{ route('admin.servers.view.manage.suspension', $server->id) }}" method="POST">
                            {!! csrf_field() !!}
                            <input type="hidden" name="action" value="unsuspend" />
                            <button type="submit" class="btn btn-success">取消暂停服务器</button>
                        </form>
                    </div>
                </div>
            </div>
                ...
                <div class="box box-success">
                    ...
                    <div class="box-body">
                        <p>
                            ...
                            <strong>警告！</strong>此功能尚未完全测试，可能存在错误。
                        </p>
                    </div>
                    ...
                    <div class="box-footer">
                        ...
                        @else
                            ...
                            <p style="padding-top: 1rem;">转移服务器需要在您的面板上配置多个节点。</p>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="col-sm-4">
                <div class="box box-success">
                    ...
                    <div class="box-body">
                        <p>
                            此服务器当前正在转移到另一个节点。
                            转移于<strong>{{ $server->transfer->created_at }}</strong>启动
                        </p>
                    </div>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group col-md-12">
                                ...
                                <p class="small text-muted no-margin">此服务器将转移到的节点。</p>
                            </div>
