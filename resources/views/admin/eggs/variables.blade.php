...
@section('content-header')
    <h1>{{ $egg->name }}<small>管理此预设的变量。</small></h1>
    <ol class="breadcrumb">
...
<div class="row">
    <div class="col-xs-12">
        <div class="box no-border">
            <div class="box-body">
                <a href="#" class="btn btn-sm btn-success pull-right" data-toggle="modal" data-target="#newVariableModal">创建新变量</a>
            </div>
        </div>
    </div>
</div>
                ...
                <form action="{{ route('admin.nests.egg.variables.edit', ['egg' => $egg->id, 'variable' => $variable->id]) }}" method="POST">
                    <div class="box-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-12">
                                <p class="text-muted small">可以通过使用<code>{{ $variable->env_variable }}</code>在启动命令中访问此变量。</p>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="form-label">权限</label>
                            <select name="options[]" class="pOptions form-control" multiple>
                                <option value="user_viewable" {{ (! $variable->user_viewable) ?: 'selected' }}>用户可查看</option>
                                <option value="user_editable" {{ (! $variable->user_editable) ?: 'selected' }}>用户可编辑</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label class="form-label">输入规则</label>
                            <input type="text" name="rules" class="form-control" value="{{ $variable->rules }}" />
                            <p class="text-muted small">这些规则使用标准的<a href="https://laravel.com/docs/5.7/validation#available-validation-rules" target="_blank">Laravel框架验证规则</a>定义。</p>
                        </div>
                    </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">创建新的预设变量</h4>
            </div>
            <form action="{{ route('admin.nests.egg.variables', $egg->id) }}" method="POST">
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-12">
                            <p class="text-muted small">可以通过输入<code>@{{environment variable value}}</code>在启动命令中访问此变量。</p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label">权限</label>
                        <select name="options[]" class="pOptions form-control" multiple>
                            <option value="user_viewable">用户可查看</option>
                            <option value="user_editable">用户可编辑</option>
                        </select>
                    </div>
                    <div class="form-group">
