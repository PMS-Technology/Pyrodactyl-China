...
const BackupContextMenu = ({ backup }: Props) => {
    ...
    return (
        <>
            <Dialog open={modal === 'rename'} onClose={() => setModal('')} title='重命名备份'>
                <div className='space-y-4'>
                    <div>
                        <label className='block text-sm font-medium text-zinc-200 mb-2'>备份名称</label>
                        <input
                            type='text'
                            value={newName}
            ...
            <Dialog open={modal === 'restore'} onClose={() => setModal('')} title='恢复备份'>
                <div className='space-y-4'>
                    <div className='space-y-2'>
                        <p className='text-sm font-medium text-zinc-200'>&quot;{backup.name}&quot;</p>
                        <p className='text-sm text-zinc-400'>
                            恢复过程中您的服务器将停止运行。在完成之前，您将无法控制
                            电源状态、访问文件管理器或创建其他备份。
                        </p>
                    </div>
                ...
                <Dialog.Footer>
                    ...
                    <ActionButton onClick={() => doRestorationAction()} variant='danger' disabled={countdown > 0}>
                        {countdown > 0 ? `删除全部并恢复 (${countdown}秒)` : '删除全部并恢复备份'}
                    </ActionButton>
                </Dialog.Footer>
            </Dialog>
            <Dialog.Confirm
