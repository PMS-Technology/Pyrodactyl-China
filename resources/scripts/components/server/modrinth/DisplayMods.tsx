...
const ProjectSelector: React.FC<Props> = ({ appVersion, baseUrl, nonApiUrl }) => {
    ...
    return (
        <div>
            ...
            {isLoading ? (
                <p className='text-white'>加载项目中...</p>
            ) : projects.length > 0 ? (
                        ...
                        <div className='flex items-center'>
                            ...
                            <div className='w-full'>
                                ...
                                <p className='text-sm text-gray-300 mb-2'>
                                    作者: <span className='font-medium'>{project.author}</span>
                                </p>
                                <p className='text-sm text-gray-400'>{project.description}</p>
                            </div>
                            <div className='flex flex-col py-2 px-6 mx-6'>
                                <p className='text-sm'>
                                    {formatNumber(project.downloads)} <span className='text-gray-600'>下载</span>
                                </p>
                                <p className='text-sm pt-2'>
                                    {project.versions.at(-1)} <span className='text-gray-600'>最新</span>
                                </p>
                            </div>
                            <div className='flex flex-col py-2 px-6 mx-6'>
                                <button
                                ...
                                >
                                    <HugeIconsDownload className='px-2 mx-2' fill='currentColor' /> 安装
                                </button>
                ...
                <p className='text-white'>暂无可用项目...</p>
            )}
        </div>
    );
};
